...
import { BatchCreateOrderDto, CreateOrderDto } from './dto/create-order.dto';
...
export class OrdersService {
  ...
  async create(
    ...
    const existingOrder = await this.orders.findOne({
      where:{
        externalOrderId: createOrderDto.order_ref
      }
    });

    if(existingOrder){
      return {message: 'Order created successfully', data: existingOrder.responsePayload}
    }
    ...
    const order = this.orders.create({
      ...
      requestPayload: JSON.stringify(normalizedInput),
    ...
    try {
      const result = await adapter.createOrder(normalizedInput, {
      requestId: requestId,
    });

    saved.responsePayload = result;
    await this.orders.save(saved);

    await this.events.save(this.events.create({
      orderId: saved.id,
      status: result.status,
      courierStatusRaw: "CREATED",
      occuredAt: new Date(),
      rawPayload: result
    }))
    ...
    return {message: 'Order created successfully', data: result};
      ...
      saved.status = ShipmentStatus.FAILED;
      saved.lastError = error.toString();
      await this.orders.save(saved);
      throw error;
    }
  ...
  async get(
    ...
    const result = await adapter.getOrder(courierOrderId, {
      requestId: requestId,
    });
    
    return {message: 'Order fetched successfully', data: result};
  }
  async cancel(
    courierOrderId: string,
    courierPartner: string,
    ...
    const adapter = this.registery.getAdapter(courierPartner);
    const result = await adapter.cancelOrder(courierOrderId, {
      requestId: requestId,
    });
    return {message: 'Order cancelled successfully', data: result};
  }

  async bulkOrderProcess(
    orders: Array<any>,
    requestId: string,
    batchId: string,
  ): Promise<void> {
    const bulkOrderCreation = orders.map((order: CreateOrderDto) =>
      this.create(order, requestId, batchId),
    );
    await Promise.all(bulkOrderCreation);
  }

  async bulkOrder(dto: BatchCreateOrderDto, requestId: string): Promise<any> {
    const batchId = uuidv4();
    const orders: CreateOrderDto[] = dto.orders;

    setImmediate(() => {
      this.bulkOrderProcess(orders, requestId, batchId);
    });

    return { message: 'Bulk order initiated successfully', data: { batchId } };
  }
}
